...
while tspan(1) < tf
    ...
    options = odeset('Events', funcion_eventos, 'AbsTol', 1e-13, 'RelTol',1e-13);%, 'MaxStep',1e-5);
    ...
    x = X_new(end, :);
     ...
     % Calculate the force being done by the rope
        ...
        if qx == 1
          FricX = Tdx_positive*x(4) + Tsx_fun_positive(x(3));
        elseif qx == -1
          FricX = Tdx_negative*x(4) - Tsx_fun_negative(x(3));    
        else
          FricX = 0;
        end
    
        if qy == 1
          FricY = Tdy_positive*x(2) + Tsy_fun_positive(x(1)); 
        elseif qy == -1
          FricY = Tdy_negative*x(2) - Tsy_fun_negative(x(1));    
        else
          FricY = 0;
        end
    ...
    % Change to the next state checking if there has been any event
        if ~isempty(IE)                                                     
                ...
                elseif (  any(IE == 3) ||  any(IE == 10)  || any(IE == 11))                                                % if it was moving and it went to speed 0
                    if (((uF_x_apl - sin(x(5))*sin(x(7))*S + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9)*sin(x(5))))) > Tsx_fun_positive(x(3))) && (x(3)<xlim_positive))         % ddxw     % dont stand still because I move from -x to +x.
                        qx = 1;
                    elseif (((uF_x_apl - sin(x(5))*sin(x(7))*S + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9)*sin(x(5))))) < -Tsx_fun_negative(x(3))) && (x(3)>xlim_negative))    % ddxw     % dont stand still because I move from +x to -x.
                        qx = -1;
                    else
                ...
                elseif (  any(IE == 6)  ||  any(IE == 12)  || any(IE == 13))                                     % if it was moving and it went to speed 0
                    if (((uF_y_apl -S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9))  ) > Tsy_fun_positive(x(1))) && (x(1)<ylim_positive))              % dont stand still because  move from -y to +y.
                        qy = 1;
                    elseif (((uF_y_apl -S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9))) < -Tsy_fun_negative(x(1))) && (x(1)>ylim_negative))         % dont stand still because  move from +y to -y.
                        qy = -1;
                    else
                ...
                elseif (  any(IE == 9) ||  any(IE == 14)  || any(IE == 15))     % if I was moving and I went to speed 0
                    ...
                    if qx == 1                  
                        dx(4) =  (uF_x_apl -(Tdx_positive*x(4) + Tsx_fun_positive(x(3))) - S*sin(x(5))*sin(x(7)) + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9)*sin(x(5)))) )/(ms + mw + IMOTx);       % ddxw               
                    elseif qx == -1             
                        dx(4) =  (uF_x_apl -(Tdx_negative*x(4) - Tsx_fun_negative(x(3))) - S*sin(x(5))*sin(x(7)) + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9)*sin(x(5)))) )/(ms + mw + IMOTx);       % ddxw           
                    else
                        dx(4) = 0;                    
                    end
                    ...
                    if qy == 1                  
                        dx(2) = (uF_y_apl - (Tdy_positive*x(2) + Tsy_fun_positive(x(1))) - S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9)) )/(mw + IMOTy);                    
                    elseif qy == -1          
                        dx(2) = (uF_y_apl - (Tdy_negative*x(2) - Tsy_fun_negative(x(1))) - S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9)) )/(mw + IMOTy);             
...
function [value, isterminal, direction] = EventsFnc(t, x, uF_x_aplicar2, uF_y_aplicar2, uF_r_aplicar2, qx, qy, qr, Tsx_fun_positive, Tsx_fun_negative , Tsy_fun_positive, Tsy_fun_negative, Tsr_fun_positive, Tsr_fun_negative, Tdy_positive,Tdy_negative, Tdx_positive, Tdx_negative, Tdr_positive, Tdr_negative , g, mc,mw,ms, xlim_positive, xlim_negative, ylim_positive,ylim_negative,rlim_positive,rlim_negative)
    ...
    f1x = ((uF_x_aplicar - sin(x(5))*sin(x(7))*S + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9))*sin(x(5))) ) - Tsx_fun_positive(x(3))) * (x(3)<xlim_positive);  
    % disp(f1x)
    
    % MOVEMENT TOWARDS -X (uF_x(t) will be negative); when it changes from + to -,
    % it starts moving towards -X (here, only the + to - change is detected)
    f2x = ((uF_x_aplicar - sin(x(5))*sin(x(7))*S + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9))*sin(x(5)))  ) + Tsx_fun_negative(x(3))) * (x(3)>xlim_negative);
    ...
    f1y = ((uF_y_aplicar -S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9))) - Tsy_fun_positive(x(1))) * (x(1)<ylim_positive);
    
    % MOVEMENT TOWARDS -Y (uF_y(t) will be negative); when it changes from + to -,
    % it starts moving towards -Y (here, only the + to - change is detected)
    f2y = ((uF_y_aplicar -S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9))) + Tsy_fun_negative(x(1))) * (x(1)>ylim_negative);
    ...
    if qx == 1                  % it moves in +X
        dx(4) =  (uF_x_aplicar - (Tdx_positive*x(4) + Tsx_fun_positive(x(3))) - S*sin(x(5))*sin(x(7)) + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9))*sin(x(5))) )/(ms + mw + IMOTx);  
    elseif qx == -1             % it moves in -X
        dx(4) =  (uF_x_aplicar - (Tdx_negative*x(4) - Tsx_fun_negative(x(3))) - S*sin(x(5))*sin(x(7)) + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9))*sin(x(5))) )/(ms + mw + IMOTx); 
    else
        dx(4) = 0;
    end
    % -- y --
    if qy == 1                  % it moves in +Y
        dx(2) = (uF_y_aplicar - (Tdy_positive*x(2) + Tsy_fun_positive(x(1))) - S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9)) )/(mw + IMOTy);
    elseif qy == -1             % it moves in -Y
        dx(2) = (uF_y_aplicar - (Tdy_negative*x(2) - Tsy_fun_negative(x(1))) - S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9)) )/(mw + IMOTy);
    else
        dx(2) = 0;
    end
    % -- rope --
...
for i = 1:size(T_total,1)
    ...
    if qr == 0                                                             
        S = -(mc*(x(9)^2*x(6)^2*mw^2 + x(9)^2*x(8)^2*mw^2 - uF_y(t)*x(9)*ms*cos(x(5)) - uF_y(t)*x(9)*mw*cos(x(5)) + x(9)^2*x(6)^2*ms*mw ...
                + x(9)^2*x(8)^2*ms*mw + (K_AIREA*x(6)*ms*sin(2*x(5)))/2 - (K_AIREB*x(8)*mw*sin(2*x(7)))/2 - x(9)^2*x(8)^2*mw^2*cos(x(5))^2 ...
                - uF_x(t)*x(9)*mw*sin(x(5))*sin(x(7)) + x(9)*g*mw^2*cos(x(7))*sin(x(5)) + Ty*x(9)*x(2)*ms*cos(x(5)) + Ty*x(9)*x(2)*mw*cos(x(5)) ...
                - x(9)^2*x(8)^2*ms*mw*cos(x(5))^2 + K_AIREA*x(6)*mw*cos(x(5))*cos(x(7))^2*sin(x(5)) + Tx*x(9)*x(4)*mw*sin(x(5))*sin(x(7)) ...
                + x(9)*g*ms*mw*cos(x(7))*sin(x(5))))/(x(9)*(mc*mw + ms*mw + mw^2 + mc*ms*cos(x(5))^2 - mc*mw*cos(x(7))^2 + mc*mw*cos(x(5))^2*cos(x(7))^2));           
    elseif qr == 1                                                          
        Tr = Tdr_positive*x(10) - Tsr_fun_positive(x(9));                   
    ...
    if qx == 0                                                              
        ...
        ddxw =  (uF_x(t) -Tx - S*sin(x(5))*sin(x(7)) + ((K_AIREA*x(6)*cos(x(5))*sin(x(7)))/x(9) + (K_AIREB*x(8)*cos(x(7)))/(x(9)*sin(x(5))))  ) /  (ms + mw + IMOTx);      % ddxw         
    end
    ...
    if qy == 0                                                              
       ...
       ddyw = (uF_y(t) -Ty - S*cos(x(5)) - ((K_AIREA*x(6)*sin(x(5)))/x(9)))/(mw + IMOTy);                             
    end
...
fx_total_trolley = zeros(size(T_total,1),1);
...
for i = 1:length(T_total)
    ...
    if estado == 0
        fx_total_trolley(i) = 0; 

    elseif estado == 1  

        Tx = Tdx_positive*X_total(i,4) + Tsx_fun_positive(X_total(i,3)); 
        
        fx_total_trolley(i) = uF_x(tiempo_actual) -Tx - S_vec_pos(i)*sin(X_total(i,5))*sin(X_total(i,7)) + (K_AIREA*X_total(i,6)*cos(X_total(i,5))*sin(X_total(i,7)))/X_total(i,9) + (K_AIREB*X_total(i,8)*cos(X_total(i,7)))/(X_total(i,9)*sin(X_total(i,5)));
...
fy_total_trolley = zeros(size(T_total,1),1);
...
for i = 1:length(T_total)
    ...
    if estado == 0
        fy_total_trolley(i) = 0; 

    elseif estado == 1  

        Ty = Tdy_positive*X_total(i,2) + Tsy_fun_positive(X_total(i,1));
        fy_total_trolley(i) = uF_y(tiempo_actual) -Ty - S_vec_pos(i)*cos(X_total(i,5)) - ((K_AIREA*X_total(i,6)*sin(X_total(i,5)))/X_total(i,9)); 
...
f_rope_total_trolley = zeros(size(T_total,1),1);
...
for i = 1:length(T_total)    
    ...
    if estado == 0      
        ...
        f_rope_total_trolley(i) = 0;       


    else 

        f_rope_total_trolley(i) = (S_vec_pos(i) - mc*cos(X_total(i,5))*ddyw_vec(i) + mc*X_total(i,9)*X_total(i,6)^2 + mc*X_total(i,9)*X_total(i,8)^2 - mc*cos(X_total(i,5))^2*X_total(i,9)*X_total(i,8)^2 - mc*sin(X_total(i,5))*sin(X_total(i,7))*ddxw_vec(i) + g*mc*cos(X_total(i,7))*sin(X_total(i,5)) );
...
plot(T_total, fx_total_trolley, 'g:', 'LineWidth', 1.5, 'DisplayName', 'total Fx on trolley' );
title('Forces X-axis');
ylabel('Froce (N)');
xlabel('Time (s)');
...
plot(T_total, fy_total_trolley, 'g:', 'LineWidth', 1.5, 'DisplayName', 'total Fy on trolley' );
title('Forces Y-axis');
ylabel('Froce (N)');
xlabel('Time (s)');
...
plot(T_total, f_rope_total_trolley, 'g:', 'LineWidth', 1.5, 'DisplayName', 'total F on rope axis' );
...
DATA_OUT = [X_total(:,1) X_total(:,2) 0.*X_total(:,2) X_total(:,3) X_total(:,4) 0.*X_total(:,4) X_total(:,5) X_total(:,6) X_total(:,7) X_total(:,8) X_total(:,9) X_total(:,10) 0.*X_total(:,10) y_abs_carga 0.*y_abs_carga  0.*y_abs_carga x_abs_carga 0.*x_abs_carga  0.*x_abs_carga z_rel_carga 0.*z_rel_carga 0.*z_rel_carga];
